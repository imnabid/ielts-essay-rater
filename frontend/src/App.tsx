...
type RetrievedChunk = { source?: string; snippet: string };
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = {
        title,
        essay_text: essay,
        target_band: targetBand === '' ? undefined : targetBand,
        top_k: topK,
      };
      const res = await axios.post<RateResponse>(`${API_BASE}/rate`, payload, {
    ...
    return (
      <div>
        ...
        <div className="mt-1 h-2.5 w-full rounded-full bg-gray-200">
          <div
            className={`h-2.5 rounded-full ${barColor(score)}`}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-semibold tracking-tight text-gray-900">IELTS Essay Rater</h1>
        <p className="text-gray-600 mt-1">Rate your Task 2 essay and get targeted recommendations.</p>
                  ...
                  <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-3">
                      <div className="h-2.5 w-full rounded-full bg-gray-200">
                        <div className={`h-2.5 rounded-full ${barColor(c.score)}`} style={{ width: `${percent(c.score)}%` }} />
                      </div>
                    </div>
